...
export default function AgendaClient({ appts = [] as Appt[] }) {
  ...
  const events = (appts ?? []).map((a) => {
    ...
    return {
      day: Math.max(0, Math.min(4, (start.getDay() + 6) % 7)), // 1=Seg ... 5=Sex
        ...
        <CardContent>
          <div className="rounded-2xl border overflow-hidden">
            <div className="grid grid-cols-[64px_repeat(5,1fr)]">
              ...
              {days.map((d, i) => (
                <div key={i} className="bg-gray-50 p-2 text-sm font-medium">
                  {d}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-[64px_repeat(5,1fr)]">
              <div className="flex flex-col">
                {hours.map((h) => (
                  <div
                    key={h}
                    className="h-16 text-xs text-gray-500 pl-2 border-b flex items-start pt-1"
                  >
              ...
              {days.map((_, dayIdx) => (
                <div key={dayIdx} className="relative">
                  {hours.map((h) => (
                    <div key={h} className="h-16 border-l border-b" />
                  ))}
                  {events
                    ...
                    .map((e, idx) => (
                      <div
                        ...
                        style={{
                          top: (e.start - 8) * 64 + 8,
                          height: (e.end - e.start) * 64 - 16,
                        }}
                      >
                        <div className="font-medium">{e.title}</div>
                        <div className="text-xs text-gray-500">
                          {String(e.start).padStart(2, "0")}:00–
                          {String(e.end).padStart(2, "0")}:00
                        </div>
                      </div>
                    ))}
                </div>
